...
# path to directory containing input edge difference table data and output
epcaresdir = Sys.getenv()['resepca']

# path to Phylosift related data
phylosiftmarkerdbdir = Sys.getenv()['refpkg']
phylosiftmarkertag = Sys.getenv()['marker']
phylosiftmarker = sprintf('%s.annotated', phylosiftmarkertag)

# path to additional data files
nfsampleref = Sys.getenv()['sampleref']
nfcoodtable = Sys.getenv()['samplecoordinates']
nfgeneticdist = Sys.getenv()['humangenetdist']

# load GPS-related function
source(file.path(Sys.getenv()['scripts'], 'gpscoords.r'))
...
## load the taxon names associated with the edges of the reference tree from NCBI Taxonomy database (using local PosgreSQL build)
...
#### NB: using ade4 implementation of PCA, the consequence is TOTAL INVERSION OF THE PRINCIPAL DRIECTION SIGN PATTERN DEPENDING ON ORIGINAL SIGN OF EDGEDIFF VALUES, quite important !!!!

# choose to have less negative edgediff values in samples enriched for the clade, by taking the opposite value of guppy output
edgediff = -1 * read.csv(paste(epcaresdir, paste(prefix, 'edgediff', sep='.'), sep='/'), row.names=1, header=F, sep=',')
#~ hist(data.matrix(edgediff))

# change sign of edges which are on the path from arbitrary root to ToL root, creating signal inversion for these edges
invertededges = read.table(sprintf('%s/%s.old2newrootpath.invertededges', epcaresdir, prefix), sep='\t', head=F, colClasses=c('numeric', 'character'))
...
sampleref = read.table(nfsampleref, sep='\t', header=T, stringsAsFactors=F)
...
gps.locs = read.table(nfcoodtable, head=T, sep='\t')
...
#~ 	print(summary(aov(lm(pcali[filipinos,3]  ~ lifestyles[filipinos]))))
#~ 	print(summary(aov(lm(pcali[filipinos,3]  ~ localities[filipinos]))))
#~ 	print(summary(aov(lm(pcali[filipinos,3]  ~ lifestyles[filipinos] * localities[filipinos]))))
#~ 	print(summary(lm(pcali[filipinos,3]  ~ localities[filipinos] * lifestyles[filipinos])))
#~ 	
#~ 	print(summary(aov(lm(pcali[filipinos,4]  ~ lifestyles[filipinos]))))
#~ 	print(summary(aov(lm(pcali[filipinos,4]  ~ localities[filipinos]))))
#~ 	print(summary(aov(lm(pcali[filipinos,4]  ~ lifestyles[filipinos] * localities[filipinos]))))
#~ 	print(summary(lm(pcali[filipinos,4]  ~ localities[filipinos] * lifestyles[filipinos])))
	
	print(summary(aov(lm(pcali[,3]  ~ lifestyles))))
	print(summary(aov(lm(pcali[,3]  ~ lifestyles * localities))))
	print(summary(aov(lm(pcali[,3]  ~ localities * lifestyles))))
	
	print(summary(aov(lm(pcali[,4]  ~ lifestyles))))
	print(summary(aov(lm(pcali[,4]  ~ lifestyles * localities))))
	print(summary(aov(lm(pcali[,4]  ~ localities * lifestyles))))
	print(ordPens::ordAOV(as.integer(lifestyles), pcali[,4]))
	...
	#~ # try my own calculations, not sure of them:
	#~ apply(disc.pcali$scaling, 2, function(ldcoef){ sum((abs(ldcoef) / sum(abs(ldcoef))) * (pcaeig[1:4] / sum(pcaeig))) })
	#~ # accounting for DA eigenvalues ??
	#~ sapply(1:nd, function(ld){ (dapcls$eig[ld]/sum(dapcls$eig)) * sum((abs(disc.pcali$scaling[,ld]) / sum(abs(disc.pcali$scaling[,ld]))) * (pcaeig[1:4] / sum(pcaeig))) })
	# use package built-in function
#~ 	print(RVAideMemoire::DA.var(disc.pcali))
	...
	genettag = 'FULL'
	hostgendist = as.dist(read.table(nfgeneticdist, skip=2, row.names=1))
	# using all data
...
# PCA made by guppy uses a diffrent projection!
### has to be wrong because does not take into account reversal of sign on Enterobacter lineage when re-rooting from arbitrary root in Enterobacter sp. leaf to Tree of Life root. 
